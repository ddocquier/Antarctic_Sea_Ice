...
Compute the error in tau_ji and R_ji based on the normal bootstrap with replacement (resampling of variables)
...
def compute_liang_nvar(x,dx,dt,n_iter):
    ...
    def compute_liang_index_norm(detC,Deltaik,Ckdi,T_all,Tii,gii,Cii,Tji):
        ...
        Z = np.sum(np.abs(T_all)) + np.abs(noise) # normalizer (equation (20))
        tau = 100. * Tji / Z # relative rate of information flowing from xj to xi (%) (equation (19))
        noise_percent = 100. * np.abs(noise)
        return tau,noise_percent
    ...
    noise = np.zeros((nvar,nvar))
    for i in np.arange(nvar):
        for j in np.arange(nvar):
            tau[j,i],noise[j,i] = compute_liang_index_norm(detC,Delta[i,:],dC[:,i],T[:,i],T[i,i],g[i],C[i,i],T[j,i]) # compute tau and create matrix
    ...
    for it in np.arange(n_iter): # loop over bootstrap realizations
        ...
        for i in np.arange(nvar):
            for j in np.arange(nvar):
                boot_tau[it,j,i],notused = compute_liang_index_norm(boot_detC,boot_Delta[i,:],boot_dC[:,i],boot_T[it,:,i],boot_T[it,i,i],boot_g[i],boot_C[i,i],boot_T[it,j,i])

    # Compute error in tau and R (standard deviation of boostraped values)
    error_tau = np.nanstd(boot_tau,axis=0)
    error_R = np.nanstd(boot_R,axis=0)
    
    # Return result of function
    return tau,R,error_tau,error_R